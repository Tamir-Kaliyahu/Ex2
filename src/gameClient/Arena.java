...
public class Arena {
	...
	public static ArrayList<CL_Pokemon> json2Pokemons(String fs) {
		...
		try {
			...
			for(int i=0;i<ags.length();i++) {
				...
				CL_Pokemon f = new CL_Pokemon(new Point3D(p), t, v, (double) 0, null);
				ans.add(f);
			}
		}
	...
	public static void updateEdge(CL_Pokemon fr, directed_weighted_graph g) {
		...
		while(itr.hasNext()) {
			...
			Iterator<edge_data> iter = g.getE(v.getKey()).iterator();//<<<omer
			while(iter.hasNext()) {
	...
	protected HashMap <Integer,HashMap<Integer,Integer>> getWays()
	{
		...
		Collection<node_data> A= this._gg.getV();
		node_data current;
...
//			itcurr = A.iterator();
    ...
    protected HashMap <Integer,HashMap<Integer,Double>> getWaysWeight()
    {
        HashMap <Integer,HashMap<Integer,Double>> H = new HashMap<>();
        Collection<node_data> A= this._gg.getV();
        node_data current;
        Iterator <node_data> it = A.iterator();
        while(it.hasNext())
